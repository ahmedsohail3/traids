...
const SOCKET_URL = Config.SOCKET_STAGING_URL;
...
export const joinConversation = (conversationId) => {
  if (!_socket?.connected || !conversationId) {
    if (__DEV__) console.warn('[Socket] joinConversation — not ready | connected:', _socket?.connected, '| id:', conversationId);
    return;
  }
  console.log('[Socket] conversationId type:', typeof conversationId, '| value:', conversationId);
  _socket.emit('joinConversation', { conversationId }, (ack) => {
    if (__DEV__) {
      if (ack?.success) {
        console.log('[Socket] Room joined ✓ — room:', ack.room, '| conv:', conversationId);
      } else {
        console.warn('[Socket] Room join FAILED — conv:', conversationId, '| ack:', JSON.stringify(ack));
      }
    }
  });
};
