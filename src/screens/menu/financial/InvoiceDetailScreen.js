...
const InvoiceDetailScreen = ({route}) => {
  ...
  const {paymentMethod, loadingPaymentMethod, getPaymentMethod} =
    useCompanyCardSetup();
        ...
        <ScrollView
          ...
          <View style={styles.invoiceCard}>
            ...
            {(inv.lineItems ?? []).map((item, idx) => (
              <View
                key={idx}
                style={[styles.contractorBlock, {borderColor: colors.border}]}>
                <Text style={styles.contractorName}>
                  {item.subcontractor?.fullName ??
                    item.subcontractorName ??
                    '—'}
                </Text>
                <Text style={styles.contractorMeta}>
                  Hourly Rate: {fmt(item.hourlyRate)}/hr
                </Text>
                <View style={styles.itemDivider} />
                <View style={styles.contractorAmtRow}>
                  <Text style={styles.contractorHours}>
                    Hours: {item.hours ?? 0}
                  </Text>
                  <Text style={styles.contractorAmt}>
            ...
            <View style={styles.feeRow}>
              <Text style={styles.feeName}>
                Platform Fee (
                {inv.lineItems?.[0]?.platformFeePercent != null
              ...
              <>
                ...
                {!loadingPaymentMethod && (
                  <Text style={[styles.cardOnFile, {color: colors.textMuted}]}>
                    {paymentMethod?.last4
                      ? `Paying with ${paymentMethod.brand ?? 'card'} •••• ${
                          paymentMethod.last4
                        }`
