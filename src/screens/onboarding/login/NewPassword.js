...
const NewPasswordScreen = ({ navigation }) => {
  ...
  const handleReset = useCallback(() => {
    ...
    else if (!allRulesPassed) newErrors.password = 'Password does not meet requirements';
  ...
  return (
    <>
      <AuthContainer>
        <Text variant='sectionTitle' style={{ color: colors.textPrimary, marginBottom: 6 }}>Create A New Password</Text>
        ...
        <TextInput
          ...
          placeholder="Enter your email" /* matching screenshot */
          secureTextEntry
        ...
        {password.length > 0 && (
          <View style={styles.rulesContainer}>
            {PASSWORD_RULES.map(r => (
              <RuleRow key={r.key} label={r.label} passed={r.test(password)} colors={colors} />
            ))}
          </View>
        )}
