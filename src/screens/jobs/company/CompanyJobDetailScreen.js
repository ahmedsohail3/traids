...
const CompanyJobDetailScreen = ({route, navigation}) => {
  ...
  const openEdit = useCallback(() => {
    setEditForm({
      ...
      workersRequired:
        job.workersRequired != null ? String(job.workersRequired) : '',
    });
    setEditDocuments([]);
    setEditMode(true);
  }, [job]);
  ...
  const handleSaveEdit = useCallback(async () => {
    try {
      await updateJob(jobId, {...editForm, documents: editDocuments});
      showAlert({
        title: 'Updated',
  ...
  const handleAccept = applicationId => {
    showConfirm({
      ...
      message:
        'Are you sure you want to accept this subcontractor application?',
      confirmText: 'Accept',
      type: 'success',
      onConfirm: async () => {
        try {
          ...
          showAlert({
            title: 'Error',
            message: err ?? 'Failed to accept application.',
            type: 'error',
          });
        }
      },
  ...
  const handleReject = applicationId => {
    showConfirm({
      ...
      message:
        'Are you sure you want to reject this subcontractor application?',
      confirmText: 'Reject',
      type: 'error',
      onConfirm: async () => {
        try {
          ...
          showAlert({
            title: 'Error',
            message: err ?? 'Failed to reject application.',
            type: 'error',
          });
        }
      },
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          showAlert({
            title: 'Job Completed',
            message: 'The job has been marked as completed.',
            type: 'success',
          });
        } catch (err) {
          showAlert({
  ...
  const statusLabel =
    STATUS_LABELS[statusKey] ?? job.status ?? status ?? 'Pending';
      ...
      <ScrollView
          ...
          {(job.timelineStartDate || job.timelineEndDate) && (
            <View style={styles.timelineRow}>
              <Text style={styles.timelineLabel}>
                {formatDate(job.timelineStartDate)} →{' '}
                {formatDate(job.timelineEndDate)}
              </Text>
            </View>
          )}
            ...
            <RNScrollView
              ...
              <TextInput
                ...
                onChangeText={v =>
                  setEditForm(f => ({
                    ...f,
                    workersRequired: v.replace(/[^0-9]/g, ''),
                  }))
                }
                placeholder="e.g. 3"
                placeholderTextColor="#94A3B8"
              ...
              {editDocuments.map((doc, i) => (
                <View key={i} style={styles.newDocRow}>
                  <Text style={styles.newDocName} numberOfLines={1}>
                    {doc.name ?? `Document ${i + 1}`}
                  </Text>
                  <TouchableOpacity
                    onPress={() => handleRemoveDoc(i)}
                    hitSlop={8}>
                    <Trash2
                      size={RFValue(14)}
                      color="#EF4444"
        ...
        {applicants.length > 0 ? (
          ...
          <View style={styles.noApplicants}>
            <Text
              style={[styles.noApplicantsText, {color: colors.textSecondary}]}>
              No applicants yet.
            </Text>
          </View>
        )}
