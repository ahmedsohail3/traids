...
const AttachmentFile = ({ uri, isSent }) => {
  ...
  return (
    <TouchableOpacity
      style={[styles.filePill, isSent ? styles.filePillSent : styles.filePillReceived]}
      onPress={() => openUrl(uri)}
      activeOpacity={0.7}>
      <FileIcon size={RFValue(13)} color={iconColor} strokeWidth={1.8} />
      <Text
        style={[styles.fileText, isSent ? styles.fileTextSent : styles.fileTextReceived]}
        numberOfLines={1}>
        {filenameFromUrl(uri)}
      </Text>
    </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  avatarPlaceholder: { alignItems: 'center', justifyContent: 'center', backgroundColor: '#10375C' },
  avatarInitial:     { fontFamily: FontFamily.semiBold, fontSize: RFValue(8), color: '#FFFFFF' },
  col:      { maxWidth: '72%' },
  ...
  metaTime:   { fontFamily: FontFamily.medium,   fontSize: RFValue(8.5), color: '#475569' },
